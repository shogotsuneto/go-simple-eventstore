...
func main() {
	...
	// Demonstrate database-generated timestamps feature
	fmt.Println("\n⏰ Demonstrating database-generated timestamps...")
	
	dbTimestampTableName := *tableName + "_db_timestamps"
	...
	fmt.Println("✅ Event store with database-generated timestamps ready")
	
	// Create events - timestamps will be generated by the database
	dbTimestampEvents := []eventstore.Event{
		{
			Type: "UserRegistered",
			Data: []byte(`{"user_id": "user-456", "username": "jane_doe"}`),
			Metadata: map[string]string{
				"source": "user-service",
				"reason": "registration",
			},
		...
		{
			Type: "UserProfileCompleted",
			Data: []byte(`{"user_id": "user-456", "profile_complete": true}`),
			Metadata: map[string]string{
				"source": "user-service",
	...
	fmt.Println("✅ Events with DB-generated timestamps appended successfully!")
	
	// Load and display the events
	fmt.Printf("\n📖 Loading events with DB-generated timestamps from stream '%s'...\n", dbTimestampStreamID)
	
	dbTimestampLoadedEvents, err := dbTimestampStore.Load(dbTimestampStreamID, eventstore.LoadOptions{ExclusiveStartVersion: 0, Limit: 10})
	if err != nil {
		log.Fatalf("Failed to load events with DB timestamps: %v", err)
	}
	
	fmt.Printf("✅ Loaded %d events with DB-generated timestamps:\n\n", len(dbTimestampLoadedEvents))
	
	for i, event := range dbTimestampLoadedEvents {
		fmt.Printf("DB Timestamp Event #%d:\n", i+1)
		fmt.Printf("  ID: %s\n", event.ID)
		fmt.Printf("  Type: %s\n", event.Type)
		fmt.Printf("  Version: %d\n", event.Version)
		fmt.Printf("  Timestamp: %s (generated by database)\n", event.Timestamp.Format("2006-01-02 15:04:05.000"))
		fmt.Printf("  Data: %s\n", string(event.Data))
		fmt.Printf("  Metadata: %v\n", event.Metadata)
		fmt.Println()
	}
	...
	fmt.Printf("💡 Database-generated timestamps were demonstrated in table '%s'.\n", dbTimestampTableName)
	fmt.Println("   The timestamps for these events were generated by PostgreSQL using DEFAULT CURRENT_TIMESTAMP.")
}
